...
then
    #input UAC
    $SIPP_PATH -sf $UAC_DESC_PATH -p $UAC_PORT -i $UAC_HOST_IP -rsa $PROXY_HOST_IP:$PROXY_PORT $UAS_HOST_IP:$UAS_PORT -trace_err -trace_msg -inf $CSV_FILE -l $L -m $M
else
    #input UAC
    $SIPP_PATH -sf $UAC_DESC_PATH -p $UAC_PORT -i $UAC_HOST_IP -rsa $PROXY_HOST_IP:$PROXY_PORT $UAS_HOST_IP:$UAS_PORT -inf $CSV_FILE -l $L -m $M
fi;
exit